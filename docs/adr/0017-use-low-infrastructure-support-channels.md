...
- A non-promotable FCM transport-proof build has one narrow, explicit local operator action after a successful matching registration. It may show only the opaque installation ID and matching Home Time Zone; it never shows a credential or push token, emits no telemetry or logs, and is absent from ordinary builds. This test artifact is not a support diagnostic.
